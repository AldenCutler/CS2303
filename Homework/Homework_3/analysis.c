...
int main(void){
    ...
    for (int i = 0; i < 6; i++){
        ...
        while(fscanf(f1, "%d %f", &new_item, &cost) > 0){
            month_data[new_item][i+1] = month_data[new_item][i+1] + 1; // increment item count
            month_data[new_item][0] = new_item; // store item ID
        }
        fclose(f1);
    ...
    get_totals(month_data, 7, ITEMS);
    get_profits(month_data, 7, ITEMS, money);
    top_earner(month_data, 7, ITEMS, money);
}
